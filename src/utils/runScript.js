        ...
        do {
            for (let i = 0; i < actor.scripts.length; i++) {
                ...
                switch (blockIdentifier) {
                    ...
                    // ==========================
                    // 🧠 ESP32 Pin Control Logic  
                    // ==========================
                    case 'setDigitalPin': {
                        const { pin = 2, value = 1 } = b;
                        if (isConnected()) {
                            try {
                                await sendCommand(`digitalWrite ${pin} ${value}`);
                                console.log(`📡 Sent digitalWrite for pin ${pin} with value ${value}`);
                            } catch (err) {
                                console.warn('sendCommand error (setDigitalPin):', err);
                            }
                        } else {
                            console.warn('⚠️ ESP32 not connected (setDigitalPin)');
                        }
                        break;
                    }
                    ...
                    case 'readDigitalPin': {
                        ...
                        if (isConnected()) {
                            try {
                                const result = await sendCommand(`digitalRead ${pin}`);
                                console.log(`📥 Read digital pin ${pin}:`, result);
                            } catch (err) {
                                console.warn('sendCommand error (readDigitalPin):', err);
                            }
                        } else {
                            console.warn('⚠️ ESP32 not connected (readDigitalPin)');
                        }
                        break;
                    }
                    ...
                    case 'readAnalogPin': {
                        ...
                        if (isConnected()) {
                            try {
                                const result = await sendCommand(`analogRead ${pin}`);
                                console.log(`📥 Read analog pin ${pin}:`, result);
                            } catch (err) {
                                console.warn('sendCommand error (readAnalogPin):', err);
                            }
                        } else {
                            console.warn('⚠️ ESP32 not connected (readAnalogPin)');
                        }
                        break;
                    }

                    case 'setPWM': {
                        const { pin = 5, dutyCycle = 128 } = b;
                        const dc = Math.max(0, Math.min(255, dutyCycle));
                        if (isConnected()) {
                            try {
                                await sendCommand(`analogWrite ${pin} ${dc}`);
                                console.log(`🎚️ Set PWM on pin ${pin} with duty cycle ${dc}`);
                            } catch (err) {
                                console.warn('sendCommand error (setPWM):', err);
                            }
                        } else {
                            console.warn('⚠️ ESP32 not connected (setPWM)');
                        }
                        break;
                    }

                    case 'setDAC': {
                        const { pin = 25, value = 128 } = b;
                        const v = Math.max(0, Math.min(255, value));
                        if (isConnected()) {
                            try {
                                await sendCommand(`dacWrite ${pin} ${v}`);
                                console.log(`🎛️ Set DAC output on pin ${pin} to value ${v}`);
                            } catch (err) {
                                console.warn('sendCommand error (setDAC):', err);
                            }
                        } else {
                            console.warn('⚠️ ESP32 not connected (setDAC)');
                        }
                        break;
                    }
